...
class AoaAlgorithm:
    ...
    def get_steering_vectors(cls,freqs,pos,az,el,**kwargs):
        ...
        for fn in range(len(freqs)):
            ...
            steering_vecs_out[fn,...] = np.exp(temp_mult)
...
#%% Synthetic data creation
    ...
    def synthesize_data(cls,freq,pos,az,el,mag=1,**kwargs):
        ...
        synth_data = np.array([np.exp(-np.matmul(pos,kv.T)) for kv in kvecs],dtype=options['dtype']) #go through all az,el angles
        #add our magnitudes
    ...
    def test_calculate_against_data(self):
        ...
        if self.options['calculated_data_path'] is not None:
            ...
            calculated_data = self.options['aoa_class'].calculate(self.freqs
                                          ,self.positions,self.meas_vals,az,el,weights=self.weights)
        else:
