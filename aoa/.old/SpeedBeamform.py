...
class SpeedBeamform:
    ...
    def get_beamformed_values(self,freqs,positions,weights,meas_vals,az,el,*args,**kwargs):
        ...
        @param[in] az - np.array of azimuthal angles to calculate in radians
        @param[in] el - np.array of elevations in radians
        @return array of beamformed value at a given frequency for a given list of azel
        '''
        raise NotImplementedError   
        
    def get_k(freq,eps_r=1,mu_r=1):
        '''
        @brief this should return a wavenumber
        @param[in] freq - frequency in Hz
        @param[in/OPT] eps_r - relative permeability
        @param[in/OPT] mu_r  - relative permittivity
        @return wavenumber k with given inputs
        '''
        raise NotImplementedError
        
    def get_k_vector_azel(self,freq,az,el):
        '''
        @brief get our k vectors (e.g. kv_x = k*sin(az)*cos(el))
        @note azel here are in radians
        @param[in] freq - frequency in Hz
        @param[in] az - azimuth angle in radians (can be numpy array)
        @param[in] el - elevation angle in radians (can be numpy array)
        @return numpy array of X,Y,Z k vectors for each given az/el
        '''
        raise NotImplementedError
...
class PythonBeamform(SpeedBeamform):
    ...
    def get_k_vector_azel(self,freq,az,el):
        ...
        >>> myPythonBeamform.get_k_vector_azel(40e9,np.deg2rad([45, 50]),np.deg2rad([0,0]))
        array([[592.79449094,   0.        , 592.79449094],
               [642.20417308,   0.        , 538.87328477]])
        '''
        k = self.get_k(freq,1.,1.)
        #print(az,el)
        kvec = k*np.array([
                np.sin(az)*np.cos(el),
                np.sin(el),
...
#%%
############################################
### beamform with subroutines using ctypes
############################################
class CtypesBeamform(SpeedBeamform):
    '''
    @brief class for subroutines interface usnig ctypes
    '''    
...
class SpeedBeamformUnittest:
    ...
    def get_angles(self):
        ...
        @return az,el
        '''
        az = np.deg2rad(np.arange(-90,90,1))
        el = np.deg2rad(np.arange(-90,90,1))
        return az,el
